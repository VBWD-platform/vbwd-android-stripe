# vbwd-android-stripe — architecture

Port of the iOS `stripe payment` plugin. Plugin id `stripe-payment`,
version `1.0.0`.
...
Stripe payment — the payment action creates a Stripe Checkout session and returns its URL to open.

Concretely it registers: the `stripe` payment action, translations — all via the `PlatformSdk` facade, never by
